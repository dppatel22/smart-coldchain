 ...
 * ─────────────────────────────────────────────────────────────────────────────
 * Controlled form that creates a new clinic via the useClinics hook.
 * Displays validation feedback inline and fires a toast on success/failure.
 *
 * Props:
...
export default function ClinicForm({ onAdd }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showToast(`"${clinic?.name ?? form.name}" added successfully!`, 'success');
      setForm(INITIAL);
      setErrors({});
    } catch (err) {
  ...
  return (
    <form className={styles.form} onSubmit={handleSubmit} noValidate>
      <h2 className={styles.title}>Register Facility</h2>
